    ...
    if test:
        ...
        with torch.no_grad():
            for step, data in enumerate(data_loader):
                ...
                if type(data[1]) == dict:
                    texts = {'roberta': data[1]['roberta'].to(device), 'wobert': data[1]['wobert'].to(device),
                             'mask': data[1]['mask'].to(device)}
                else:
        ...
        for step, data in loop:
            labels=data[0].to(device)
            if type(data[1])==dict:
                texts={'roberta':data[1]['roberta'].to(device),'wobert':data[1]['wobert'].to(device),'mask':data[1]['mask'].to(device)}
            else:
                texts=data[1].to(device)
